(() => {
  ...
  function renderGallery(results) {
    gallery.innerHTML = '';
    // Limit to maximum 8 results for better UX
    ...
    displayResults.forEach((r, idx) => {
      ...
      const badge = r.error ? 'خطأ' : `${cleanlinessPercent}% نظيف`;
      ...
      div.innerHTML = `
        <div style="position:relative">
          ...
          <img class="result-image" src="${src}" alt="صورة الشاطئ ${r.index + 1}" loading="lazy" />
        </div>
        <div class="result-details">
          <div class="result-label">${r.error ? 'خطأ' : r.imageLabel || 'غير معروف'}</div>
          <div class="result-meta">
            ${r.error ?
              `<span class="error-text">${r.error}</span>` :
              `<span class="waste-count">${r.wasteCount} عناصر قمامة مكتشفة</span>`
            }
          </div>
          ${!r.error ? `<div class="confidence-info">ثقة الذكاء الاصطناعي: ${r.confidences && r.confidences.length > 0 ? `${(r.confidences.reduce((a,b)=>a+b,0)/r.confidences.length * 100).toFixed(1)}%` : 'غير متوفر'}</div>` : ''}
        </div>
      `;
